...
- (void)viewDidLoad {
    ...
    _imageArr = @[@"输入手机号",@"输入密码"];
    _placeholderArr = @[@"请输入账号",@"请输入密码"];
...
- (void)createUI {
    ...
    [registerBtn mas_makeConstraints:^(MASConstraintMaker *maker){
        maker.top.equalTo(lastLine.mas_bottom).equalTo(@40);
        maker.left.equalTo(self.view).offset(35);
        maker.right.equalTo(self.view).offset(-35);
...
- (void)clickRegisterBtn {
    if ([_account.text isEqualToString:@""] || [_verifycode.text isEqualToString:@""]) {
    ...
    [MedicineBoxNetMethod boxAFPostWay:@"api/verif/check_verif" HeaderToken:nil PostDic:@{@"phone":_account.text,@"verif":_verifycode.text} Completion:^(NSDictionary *dic){
        ...
        if ([dic[@"resultCode"] intValue] == 1) {
            //second deliver retrive password refer data to reset vc
            ResetPasswordViewController *resetVC = [[ResetPasswordViewController alloc]init];
            resetVC.dic = @{@"phone":_account.text,@"verif":_verifycode.text};
...
- (void)clickGetCaptchaBtn {
    ...
    if (!isMatch) {
        ...
        //get verifycode api
        [MBProgressHUD showHUDAddedTo:self.navigationController.view animated:YES];
        [MedicineBoxNetMethod boxAFPostWay:@"api/verif/gen_verif" HeaderToken:nil PostDic:@{@"phone":_account.text,@"way":@"fgt_pwd"} Completion:^(NSDictionary *dic){
            [MBProgressHUD hideHUDForView:self.navigationController.view animated:YES];
